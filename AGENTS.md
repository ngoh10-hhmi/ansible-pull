...
- Ubuntu 26.04 ships SSSD as a non-root service running under the package-provisioned `sssd` user. The role detects this by checking for the `sssd` group and, when present, sets `/etc/krb5.keytab` and `/etc/sssd/sssd.conf` to `root:sssd` mode `0640` so the daemon can read them. The legacy `root:root 0600` permissions still apply on releases where the `sssd` group is absent. Any future Kerberos-adjacent file the role manages (e.g. `/etc/sssd/conf.d/*` drop-ins) must be ownership-gated the same way or SSSD will fail to start on 26.04.
