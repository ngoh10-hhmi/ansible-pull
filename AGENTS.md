...
- Ubuntu 26.04 ships SSSD as a non-root service running under the package-provisioned `sssd` user. The role gates this behavior on Ubuntu 26.04 specifically, confirms the `sssd` group exists on that release, and sets `/etc/krb5.keytab` and `/etc/sssd/sssd.conf` to `root:sssd` mode `0640` so the daemon can read them. The legacy `root:root 0600` permissions apply on older releases even if an `sssd` group exists locally. Any future Kerberos-adjacent file the role manages (e.g. `/etc/sssd/conf.d/*` drop-ins) must be ownership-gated the same way or SSSD will fail to start on 26.04.
