...
During bootstrap it will ask for machine type, an AD username plus hidden password for the required `hhmi.org` domain join, optional usernames that should be added to the local `sudo` group after the join completes, and an optional Slack webhook URL (unless you pass `--slack-webhook`; leave the prompt blank to skip). It then shows a summary of the machine-identity choices and asks you to confirm. The AD username can be entered with or without the `@hhmi.org` realm suffix (case-insensitive); bootstrap strips it before calling `kinit`, and rejects any other `@realm` to avoid silently joining the wrong domain. If the machine is already domain-joined (e.g. bootstrap is being re-run), the script detects this, skips the AD credential prompt and the reboot warning, and performs a single converge with `base_ad_enroll: true`. The exception is a rename: if the hostname you enter differs from the name the machine is joined to AD as, bootstrap offers to `realm leave hhmi.org` and re-enroll under the new name, or else aborts before converging (see [docs/troubleshooting.md](docs/troubleshooting.md)).

Re-running bootstrap preserves values already in `/etc/ansible/pull.env` (the Slack webhook and the selected branch/playbook) unless you override them with the matching flag, so a re-run will not silently wipe operator-configured settings. Pass `--reset-env` to rebuild `pull.env` from flags and defaults instead.
